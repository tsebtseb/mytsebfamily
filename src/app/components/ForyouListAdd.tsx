...
export default function ForYouListAdd() {
  const pathname = usePathname();
  const [data, setData] = useState<FamilyList[]>([]);
  const [text, setText] = useState("");
  const [selectedItem, setSelectedItem] = useState<FamilyList | null>(null);
  const [isOpen, setOpen] = useState<boolean>(false);

  useEffect(() => {
    if (pathname === "/home/dad") {
      setData(familyData.filter((item) => item.for === "daddy"));
      setText("Dad");
    } else if (pathname === "/home/mom") {
      setData(familyData.filter((item) => item.for === "mommy"));
      setText("Mom");
    } else if (pathname === "/home/timer") {
      setData(familyData.filter((item) => item.for === "timer"));
      setText("Timer");
    } else if (pathname === "/home/best") {
        setData(familyData.filter((item) => item.for === "best"));
        setText("Best");
    }else {
      setData([]);
    }
